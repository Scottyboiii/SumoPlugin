...
public class SetWinner implements SubCommand {

    private final LocationSQL locationSQL;
    private final SavedLocManager savedLocManager;

    public SetWinner(LocationSQL locationSQL, SavedLocManager savedLocManager) {
    ...
    public void run(CommandSender sender, String[] args) {
        ...
        if (this.locationSQL.locationExist(LocationType.WINNER.getDatabaseName())) {
            p.sendMessage(Language.Message_Set_Winner_Exist);
            return;
        }
        this.savedLocManager.saveLocation(LocationType.WINNER.getDatabaseName(), p.getLocation());
        p.sendMessage(Language.Message_Set_Winner_Completed);
    }
