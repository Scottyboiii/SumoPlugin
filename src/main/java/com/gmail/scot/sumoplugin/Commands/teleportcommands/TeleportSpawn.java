...
public class TeleportSpawn implements SubCommand {
    ...
    public TeleportSpawn(LocationSQL locationSQL) {
    ...
    public void run(CommandSender sender, String[] args) {
        ...
        if (this.locationSQL.getLocations().size() < 1) {
            p.sendMessage(Language.Message_Spawn_Most_Be_Set);
            return;
        }
        Location location = this.locationSQL.locationTeleport(LocationType.JOIN.getDatabaseName());
        p.teleport(location);
        p.sendMessage(Language.Message_Player_Tp_Spawn);
    }
