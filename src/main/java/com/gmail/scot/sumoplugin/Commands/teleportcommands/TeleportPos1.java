...
public class TeleportPos1 implements SubCommand {

    private final LocationSQL locationSQL;

    public TeleportPos1(LocationSQL locationSQL) {
    ...
    public void run(CommandSender sender, String[] args) {
        ...
        if (this.locationSQL.getLocations().size() < 1) {
            ...
            p.sendMessage(Language.Message_Pos1_Most_Be_Set);
            return;
        }
        Location location = this.locationSQL.locationTeleport(LocationType.POS1.getDatabaseName());
        p.teleport(location);
        p.sendMessage(Language.Message_Player_Tp_Pos1);
    }
